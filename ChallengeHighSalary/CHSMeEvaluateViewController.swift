//
//  CHSMeEvaluateViewController.swift
//  ChallengeHighSalary
//
...
class CHSMeEvaluateViewController: UIViewController, UIScrollViewDelegate, UITextViewDelegate {

    let rootScrollView = TPKeyboardAvoidingScrollView()
    let textArr = ["对公司的现象","对面试官的印象","对企业文化的现象"]
...
//         scrollView
    ...
    func addTextField(){
        ...
        placeholder.frame = CGRect(x: 10, y: 170, width: screenSize.width - 20, height: 20)
    ...
    func clickSureButton(){
        ...
        if (arr[0] as! EvaluateView).num == 0 {
            ...
            checkCodeHud.label.text = "请对公司印象进行评价"
            checkCodeHud.hide(animated: true, afterDelay: 1)
            return
        }
        if (arr[1] as! EvaluateView).num == 0 {
            checkCodeHud.mode = .text
            checkCodeHud.label.text = "请对面试官印象进行评价"
            checkCodeHud.hide(animated: true, afterDelay: 1)
            return
        }
        if (arr[2] as! EvaluateView).num == 0 {
            checkCodeHud.mode = .text
            checkCodeHud.label.text = "请对企业文化印象进行评价"
            checkCodeHud.hide(animated: true, afterDelay: 1)
            return
        }

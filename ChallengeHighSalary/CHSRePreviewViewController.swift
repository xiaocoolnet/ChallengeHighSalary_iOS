...
class CHSRePreviewViewController: UIViewController {
    ...
    func setSubviews() {
        ...
        let titleLab = UILabel(frame: CGRectMake(8, 0, screenSize.width, kHeightScale*35))
        ...
        // MARK: 概况
        let summaryView = UIView(frame: CGRectMake(10, CGRectGetMaxY(titleLab.frame), screenSize.width-20, kHeightScale*174))
        ...
        addressBtn.setTitle(CHSUserInfo.currentUserInfo.city, forState: .Normal)
        ...
        expBtn.setTitle(CHSUserInfo.currentUserInfo.work_life, forState: .Normal)
        ...
        eduBtn.setTitle(CHSUserInfo.currentUserInfo.education?.first?.degree, forState: .Normal)
        ...
        propertyBtn.setTitle(CHSUserInfo.currentUserInfo.work_property, forState: .Normal)
        ...
        headerImg.sd_setImageWithURL(NSURL(string: kImagePrefix+CHSUserInfo.currentUserInfo.avatar), placeholderImage: nil)
        ...
        let salaryAttrStr = NSMutableAttributedString(string: "￥ \(CHSUserInfo.currentUserInfo.wantsalary)", attributes: [NSForegroundColorAttributeName:UIColor(red: 102/255.0, green: 102/255.0, blue: 102/255.0, alpha: 1),NSFontAttributeName:UIFont.systemFontOfSize(15)])
        ...
        summaryView.frame.size.height = CGRectGetMaxY(intentionTagLab_3.frame)+10
        ...
        for (i,eduModel) in CHSUserInfo.currentUserInfo.education!.enumerate() {
        ...
        for (i,city) in ((CHSUserInfo.currentUserInfo.work?.first?.skill.componentsSeparatedByString("-"))! ?? [String]()).enumerate() {
...
//            skillLab.frame.origin.x = jobView.frame.size.width-8-skillLab.frame.size.width
            ...
            if i+1 < ((CHSUserInfo.currentUserInfo.work?.first?.skill.componentsSeparatedByString("-"))! ?? [String]()).count {
                
                let nextBtnWidth = calculateWidth(((CHSUserInfo.currentUserInfo.work?.first?.skill.componentsSeparatedByString("-"))! ?? [String]())[i+1], size: 15, height: cityBtnHeight)+cityBtnMargin
